...
> [!WARNING]
> The `aqie-laqm-data-explorer` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `aqie-laqm-data-explorer`. To turn it
...
```bash
docker build --target development --no-cache --tag aqie-laqm-data-explorer:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 aqie-laqm-data-explorer:development
```
...
```bash
docker build --no-cache --tag aqie-laqm-data-explorer .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 aqie-laqm-data-explorer
```
...
```bash
docker run -it --entrypoint /bin/ash aqie-laqm-data-explorer
```
